...
export default function Chat() {
  ...
  const handleRename = async (id: string, newTitle: string) => {
    if (newTitle.trim()) {
      try {
        await axios.put(`https://crimevision-aq07.onrender.com/api/chat/conversations/${id}`, { title: newTitle }, getAuthHeaders());
        fetchConversations();
      } catch (e) {
  ...
  const fetchConversations = async () => {
    try {
      const res = await axios.get('https://crimevision-aq07.onrender.com' + '/api/chat/conversations', getAuthHeaders());
  ...
  const loadConversation = async (id: string) => {
    setSessionId(id);
    try {
      const res = await axios.get(`https://crimevision-aq07.onrender.com/api/chat/conversations/${id}`, getAuthHeaders());
  ...
  const performDelete = async () => {
    ...
    try {
      await axios.delete(`https://crimevision-aq07.onrender.com/api/chat/conversations/${deleteConfirmId}`, getAuthHeaders());
      fetchConversations();
