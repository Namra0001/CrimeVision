...
export default function Heatmap() {
  ...
  useEffect(() => {
    if (selectedDistrict) {
      fetch(`https://crimevision-aq07.onrender.com/api/map/police-stations?district_id=${selectedDistrict}`)
        .then(res => res.json())
  ...
  const fetchData = () => {
    let url = new URL(`https://crimevision-aq07.onrender.com/api/map/${viewMode === 'cluster' ? 'crime-location' : 'heatmap'}`);
